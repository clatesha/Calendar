//April 2, 2020
//Cheron Fletcher
//This program prints a calendar for the given year



// comments
//Question 1 - yes
// given the month, returns true if day is between 1-31 for months 1,3,5,7,8,10,and 12.
// returns true if day is between 1-30 for months 4,6,9,11
//if month is Feb, test for leap year and returns true if day is between 1-29 for leap year
//and returns true if day is between 1-28 when not a leap year

//Question 2 - yes
// test if date entered is valid, if invalid prints out that it is NOT valid
// if valid prints out the day of the week that correspondes to the month, date, and year entered

//Question 3 - based on the month and year given, determines if is a leap year and
// gives how many days are in that month
...
int main()
{
	...
	do
	{
		...
		case '4': // print out the calendar of specified year to yourname.txt file
			...
			cin >> year;
			if (print_calendar(year, "Cheron.txt")) {
				cout << "The calandar year " << year << " has been printed to Cheron.txt" << endl;
			}
			else
				cout << "The calandar did NOT print to file" << endl;
			break;
...
bool print_calendar(int year, string fileName)
{
	...
	for (int i = 1; i <= 12; i++) {
		print_calendar(i, year, outStream);
		outStream << endl;
	}
	// close file
	outStream.close();
